...
class Flow(nn.Module):
    def __init__(self, in_channel, filter_size=256, affine=True, conv_lu=True):
        super().__init__()
        ...
        self.coupling = AffineCoupling(in_channel, filter_size=filter_size, affine=affine)
...
class Block(nn.Module):
    def __init__(self, in_channel, n_flow, split=True, filter_size=256, affine=True, conv_lu=True):
        super().__init__()
        ...
        for i in range(n_flow):
            self.flows.append(Flow(squeeze_dim, filter_size=filter_size, affine=affine, conv_lu=conv_lu))
...
class Glow(nn.Module):
    def __init__(
        ...
        for i in range(n_block - 1):
            self.blocks.append(Block(n_channel, n_flow, filter_size=filter_size, affine=affine, conv_lu=conv_lu))
            n_channel *= 2
    ...
    def forward(self, input):
        ...
        out, sldj = self.dequantization(input, sldj)
...
class Dequantization(nn.Module):
    ...
    def __init__(self, alpha=1e-6, quants=256, add_unif_noise=False):
        """
        Inputs:
