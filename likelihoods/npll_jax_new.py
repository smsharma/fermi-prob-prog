...
@partial(jit, static_argnums=(6,7,))
def log_like_np_batch(theta, pt_sum_compressed, npt_compressed, data_s, f_ary, df_rho_ary, k_max, npixROI):
    """ Organize and combine non-Poissonian likelihoods across multiple templates
    """
    ...
    dnds_ary = vmap(dnds, in_axes=(None,0))(s_ary, theta)
    x_m, x_m_sum = return_x_m_vmapped(f_ary, df_rho_ary, npt_compressed, None, s_ary, dnds_ary, k_max)
    
    x_m = jnp.sum(x_m, axis=0)
    x_m_sum = jnp.sum(x_m_sum, axis=0)
    
    return log_like_internal_batch(pt_sum_compressed, data_s, x_m, x_m_sum, k_max, npixROI)


@partial(jit, static_argnums=(4, 5))
def log_like_internal_batch(pt_sum_compressed, data_s, x_m_ary, x_m_sum, k_max, npixROI):
    """Non-Poissonian likelihood for single template, given x_m and x_m_sum"""
    ...
    for k in np.arange(2, k_max + 1):
        ...
        n = jnp.arange(k - 1)
        pk_ary = pk_ary.at[:, k].set(
            jnp.sum((k - n) / k * x_m_ary[:, k - n] * pk_ary[:, n], axis=1)
            + f1_ary * pk_ary[:, k - 1] / k
        )

    get_ll_tot = lambda data: jnp.log(pk_ary[jnp.arange(npixROI), data]).sum()
    return vmap(get_ll_tot)(data_s)