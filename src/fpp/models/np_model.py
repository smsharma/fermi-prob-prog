...
class NPModel:
    ...
    def fit_svi(
        ...
        **model_static_kwargs
        ...
        #=== svi ===
        self.svi = SVI(self.model_debug, self.guide, optimizer, loss)
    ...
    def model_debug(self, data=...):
        ...
        for temp, temp_label in zip(temps, temp_labels):
            ...
            if temp_label in ["pib"]:
                ...
                A_temp = S_temp / jnp.mean(temp_pib_mod[~self.nm])
                mu += A_temp * temp_pib_mod  
            else:
                A_temp = S_temp / jnp.mean(temp[~self.nm])
                mu += A_temp * temp     
                                            
        gamma_ps = numpyro.sample("gamma_ps", dist.Uniform(0.2, 2.))
        gamma_poiss = numpyro.sample("gamma_poiss", dist.Uniform(0.2, 2.))

        temp_gce_nfw_ps = self.nfw_temp_gen.get_NFW2_template(gamma=gamma_ps)
        temp_gce_nfw_poiss = self.nfw_temp_gen.get_NFW2_template(gamma=gamma_poiss)
        ...
        theta_bulge_poiss = numpyro.sample("theta_bulge_poiss", dist.Dirichlet(jnp.ones((self.n_blg,)) / self.n_blg))
        temp_bulge = jnp.sum(theta_bulge_poiss[:, None] * self.blg_s, 0)
        
        # Normalize to same mean
        A_gce_nfw = S_gce / jnp.mean(temp_gce_nfw_poiss[~self.nm])
        A_gce_bulge = S_gce / jnp.mean(temp_bulge[~self.nm])
        temp_gce_poiss = (1 - f_bulge_poiss) * A_gce_nfw * temp_gce_nfw_poiss \
                            + f_bulge_poiss * A_gce_bulge * temp_bulge
        
        A_gce = S_gce / jnp.mean(temp_gce_poiss[~self.nm])
        mu += A_gce * temp_gce_poiss
        
        # Get mixed bulge template
        theta_bulge_ps = numpyro.sample("theta_bulge_ps", dist.Dirichlet(jnp.ones((self.n_blg,)) / self.n_blg))
        temp_bulge = jnp.sum(theta_bulge_ps[:, None] * self.blg_s, 0)

        # Normalize to same mean
        A_gce_nfw = 1 / jnp.mean(temp_gce_nfw_ps[~self.nm])
        A_gce_bulge = 1 / jnp.mean(temp_bulge[~self.nm])
        ...
        for ips, ps in enumerate(["gce", "dsk"]):
            ...
            A = Sps / jnp.mean(npt_compressed[ips][~self.nm] * jnp.trapz(s_ary * dnds_ary, s_ary))
