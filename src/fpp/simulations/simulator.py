...
def simulator(
    ...
    while True:
        ...
        sm = SimulateMap(temps_poiss, norms_poiss, [s_arr] * len(temps_ps), dnds_arr, temps_ps, psf_r_func, exp_map_norm, mask_roi=mask_roi, psf_scheme=psf_scheme)
        ...
        # map_out = map_out.reshape((1, -1))

        if (np.sum(map_out) == 0) or np.sum(np.isnan(map_out)) or np.sum(np.isinf(map_out)):
            logger.info("Reruning simulator because of bad map")
        else:
            break
...
def simulator_for_model_p6v11(m, vd, sim_all=False, delta_psf=False, flat_exposure=False):
    ...
    mask_norm = np.array(m.normalization_mask)
    ...
    return simulator(theta, temps_poiss, temps_ps, mask_sim, mask_norm, mask_roi, psf_r_func, exp_map, psf_scheme=psf_scheme)[0]
...
def simulator_for_cmp(m, vd, **kwargs):
    ...
    return simulator(theta, temps_poiss, temps_ps, mask_sim, mask_norm, mask_roi, psf_r_func, exp_map, psf_scheme=psf_scheme, sim1b=True)[0]