...
def ll_justSps(m, vd, data):
    ...
    print("mu_masked", mu_masked.shape)
    print("npt_compressed_masked", npt_compressed_masked.shape)
    print("data_masked", data_masked.shape)
    print("theta", theta)
    print("m.f_ary", m.f_ary.shape)
    print("m.df_rho_div_f_ary", m.df_rho_div_f_ary.shape)
    print("m.k_max", m.k_max)
    print("len(mu_masked)", len(mu_masked))
    ll_arr = log_like_np(theta, mu_masked, npt_compressed_masked, data_masked, m.f_ary, m.df_rho_div_f_ary, m.k_max, len(mu_masked))
    return jnp.sum(ll_arr)
