...
\begin{abstract}
    Analysis of $\gamma$-ray data towards the Galactic Center involves a subtle interplay between observations and theoretical models; the complexity of cosmic-ray transport processes in the region, combined with the fact that we only see a two-dimensional projection of this inherently three-dimensional process, means that we typically cannot make precise predictions about crucial emission mechanisms. Consequently, scientific conclusions can depend strongly on specific assumptions made about the modeling of the signal and background, as well as choices involved in the data analysis pipeline. In this work, we use differentiable probabilistic programming, enabled by recent advances in machine learning, in order to account for the large model-space inherent to $\gamma$-ray analyses in a principled manner. Focusing on the ongoing mystery of the Galactic Center Excess (GCE), we construct a differentiable forward model and likelihood that liberally uses GPU acceleration and vectorization in order to simulatneously account for a range of possible spatial morphologies consistent with the Excess emission. Our setup allows for efficient inference over the large model space using variational methods. The goal of this wok is enable analyses that are more robust to specific modeling choices compared to traditional techniques, and to provide a framework for future analyses.
\end{abstract}
...
In this work, we use differentiable probabilistic programming to alleviate this major shortcoming of traditional likelihood-based fitting techniques for analyzing $\gamma$-ray data. Using GPU acceleration and vectorization, we construct a forward model for $\gamma$-ray emission in the Galactic Center region that can generate high-resolution spatial templates for various emission components, including those associated with the Galactic Center excess signal, on the fly. This allows for analyses that are robust to specific analysis choices, and opens up the possibility of performing model-independent analyses using the same framework. In the broader context of astrophysics, this work aims to motivate the use of differentiable probabilistic programming as a way to enable high-dimensional Bayesian inference for complex descriptions of diverse astrophysical data.
...
We describe our forward model and likelihood, its implementation as a differentiable probabilistic program, as well as the inference strategies we use to do Bayesian posterior inference over the model parameters characterizing various emission mechanisms for Galactic Center $\gamma$-ray data.

\subsection{The likelihood and forward model}

\SM{Brief description of regular template fitting as well as NPTF.}

\SM{Brief description of various templates, and different degrees of freedom.}

\SM{Specifics of our forward model; hybrid template descriptions.}
